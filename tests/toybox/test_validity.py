...
def _nmse(y, y_hat):
    return 100 * (1/(len(y) * np.std(y))) *  (y - y_hat)**2
...
ODEs = [t1, t2, t3]
integrators = [rk4(), scipy(solve_ivp, {'method': 'RK45'}), scipy(odeint)]
...
def test_valid_results(I, T):
    ...
    data = system.simulate(td, w0=T['w0'], x_t=T['x_t'])
    y_hat = data['y1']
